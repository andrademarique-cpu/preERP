...
class FingerRobotConfig:
    ...
    actuators: dict[str, ActuatorSpecs] = field(default_factory=lambda: {
        "act_joint_1": ActuatorSpecs(
            joint="joint_1", kp=2.0, kv=0.025, force_max=0.30,
            armature=4e-5, damping=1e-3, tau=0.004,
        ),
        "act_joint_2": ActuatorSpecs(
            joint="joint_2", kp=0.8, kv=0.010, force_max=0.15,
            armature=2e-5, damping=8e-4, tau=0.004,
        ),
    })